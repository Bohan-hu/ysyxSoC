...
class ysyxSoCFull(implicit p: Parameters) extends LazyModule {
  ...
  // simMem := AXI4Buffer() := AXI4Fragmenter() := xbar
  val dummyNode = AXI4SlaveNodeGenerator(p(ExtBus), ChipLinkParam.allSpace)
  dummyNode := xbar
